...
class vector
{
  ...
  public:
    ...
    void insert_by_pos(const size_t &pos,const T& element){
      insert(begin(+pos,element);
    }
    void erase_by_pos(const size_t &pos){
      erase(begin()+pos);
    }
    void erase_by_value(const T& element){
      int pos = index_of(element);
      if (i!=-1)erase(begin()+pos);
    }
    bool contain(const T& element){
      int con = index_of(element);
      if (con == -1){
        return false;
      }
      return true;
    ...
    // สร้าง 1.insert by pos  2.erase by pos 3.erase by value 4.contain 5. index of 6.  
};
